---
name: code-review
description: Use when the user explicitly asks for a code review. Two modes — (A) self-review of code Claude wrote in this session, triggered by cues like "review what you just wrote", "audit that", "self-review", "check your last change"; (B) review of code the user brings in via paste, diff, PR link, or quality questions like "is this good", "any issues", "what's wrong with this". Applies CLAUDE.md rules (Simplicity First, Surgical Changes, Naming, Complexity Check, Verify don't declare, Documentation). When the language matches another available skill (csharp-avalonia, csharp-rest-aspnetcore, csharp-efcore-sqlite, csharp-windows-service, csharp-sensor-pipeline, java-sensor-pipeline), invoke it via the Skill tool to extend the checklist. Do not fire automatically — wait for an explicit review request.
---
